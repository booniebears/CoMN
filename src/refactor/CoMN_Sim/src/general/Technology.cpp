...
namespace CoMN {
Technology::Technology() { initialized = false; }

void Technology::Initialize(int _featureSizeInNano,
  ...
  if (transistorType == Conventional) {
    if (featureSizeInNano == 130) {
      ...
      cout << "Error: CMOS Technology node " << featureSizeInNano
           << "nm is not supported" << endl;
      exit(-1);
    }
    ...
    if (featureSizeInNano == 22) {
      if (deviceRoadmap == HP) {
        /* Monolayer MoS2 Transistor Beyond the Technology
         * Roadmap, 10.1109/TED.2012.2218283 */
        // FIXME: currentOnNmos/currentOnPmos/currentOffNmos/currentOffPmos at
        // temperatures other than 300K
        vdd = 0.8;
        vth = 0.2614;
      ...
      cout << "Error: 2D FET Technology node " << featureSizeInNano
           << "nm is not supported" << endl;
      exit(-1);
    }
    ...
    if (featureSizeInNano == 22) {
      ...
      cout << "Error: TFET Technology node " << featureSizeInNano
           << "nm is not supported" << endl;
      exit(-1);
    }
  }
  if (featureSizeInNano >= 22) {
    ...
    capOverlap = 0; // capOverlap and capFringe are included in capIdealGate in
                    // FinFET technology, so we let these two parameters 0
  }
  double cjd = 1e-3; /* Bottom junction capacitance, Unit: F/m^2*/
  double cjswd =
      2.5e-10; /* Isolation-edge sidewall junction capacitance, Unit: F/m */
  double cjswgd =
      0.5e-10;         /* Gate-edge sidewall junction capacitance, Unit: F/m */
  double mjd = 0.5;    /* Bottom junction capacitance grating coefficient */
  double mjswd = 0.33; /* Isolation-edge sidewall junction capacitance grading
                          coefficient */
  double mjswgd =
      0.33; /*   Gate-edge sidewall junction capacitance grading coefficient */
